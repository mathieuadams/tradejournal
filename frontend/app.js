...
function rangeCtl() {
  ...
  return `<div class="seg" role="group" aria-label="Period">${o.map(([k, l]) => `<button data-range="${k}" aria-pressed="${S.range === k}">${l}</button>`).join('')}</div>`;
}
...
function stats(ts) {
  ...
  const daily = {}; for (const t of ts) daily[t.date] = (daily[t.date] || 0) + t.net;
...
/* ---------- dashboard ---------- */
function emptyState() {
  return `<div class="panel cta-empty"><h2>No trades yet</h2><p>Import a CSV from your broker or connect Alpaca to sync fills automatically.</p>
    <a class="btn primary" href="#import" style="text-decoration:none">Import trades</a> <a class="btn" href="#settings" style="text-decoration:none">Connect Alpaca</a></div>`;
}
function vDashboard() {
  const ts = closed(scoped());
  if (!S.trades.length) return head('Where your money went', '', '') + emptyState();
  const St = stats(ts); const L = leaks(ts); const leakTotal = sum(L.buckets.filter(b => b.val < 0).map(b => b.val));
  const line = L.clean > 0 ? `Your clean trades made ${money(L.clean, false)}. Leaks cost you ${money(-leakTotal, false)}.` : `Leaks cost you ${money(-leakTotal, false)} in this period.`;
  const rows = []; let cum = L.clean;
  rows.push({ label: 'Clean trades before 11:00', n: L.cleanN, from: 0, to: L.clean, val: L.clean });
  for (const b of L.buckets) { rows.push({ label: b.label, n: b.n, from: cum, to: cum + b.val, val: b.val }); cum += b.val; }
  rows.push({ label: 'You kept', from: 0, to: cum, val: cum, total: true });
  ...
  const wf = rows.map((r, i) => `<div class="wf-row${r.total ? ' total' : ''}"><div class="wf-label">${r.label}${r.n != null ? `<small>${r.n} trade${r.n === 1 ? '' : 's'}</small>` : ''}</div>
    <div class="wf-track"><div class="wf-zero" style="left:${pct(0)}%"></div><div class="wf-bar grow ${r.total ? 'total' : r.val >= 0 ? 'pos' : ''}" style="left:${pct(Math.min(r.from, r.to))}%;width:${Math.max(.6, Math.abs(r.to - r.from) / span * 100)}%;animation-delay:${i * 70}ms"></div></div>
    <div class="wf-val ${cls(r.val)}">${money(r.val)}</div></div>`).join('');
  const W = worstLeak(ts);
  return head('Where your money went', periodText(ts)) + `
  <section class="hero" aria-label="Leak ledger"><p class="hero-line">${line}</p>
    <p class="hero-sub">Each trade counts once. A trade with a mistake tag goes to that leak; untagged trades opened after 11:00 go to the late-session leak.</p>
    <div class="wf">${wf}</div></section>
...
function equity(ts) {
  ...
  const daily = {}; for (const t of ts) daily[t.date] = (daily[t.date] || 0) + t.net;
  const ds = Object.keys(daily).sort(); let c = 0; const pts = [0, ...ds.map(d => c += daily[d])];
  const w = 600, h = 200, p = 8, lo = Math.min(...pts), hi = Math.max(...pts), sp = hi - lo || 1;
  const X = i => p + i / (pts.length - 1 || 1) * (w - 2 * p), Y = v => h - p - (v - lo) / sp * (h - 2 * p);
  const path = pts.map((v, i) => (i ? 'L' : 'M') + X(i).toFixed(1) + ' ' + Y(v).toFixed(1)).join(' '); const end = pts[pts.length - 1];
  return `<svg viewBox="0 0 ${w} ${h}" role="img" aria-label="Cumulative net P&L by day, ending at ${money(end)}" style="width:100%;height:auto">
    ...
    <path d="${path}" fill="none" stroke="${end >= 0 ? 'var(--gain)' : 'var(--loss)'}" stroke-width="2.2" stroke-linejoin="round"/></svg>
...
function prop() {
  ...
  const fut = closed(S.trades).filter(t => t.acct === P.account && (!P.start || t.date >= P.start)).sort(chron);
  const days = {}; for (const t of fut) days[t.date] = (days[t.date] || 0) + t.net;
  ...
  const lastDay = fut.length ? fut[fut.length - 1].date : null, today = lastDay ? days[lastDay] : 0; const bpct = P.trailing ? Math.max(0, Math.min(100, buffer / P.trailing * 100)) : 0;
...
function vJournal() {
  ...
  const days = [...new Set([todayNY, ...S.trades.map(t => t.date)])].sort().reverse(); if (!S.jDay) S.jDay = days[0];
  const d = S.daily[S.jDay]; const ts = S.trades.filter(t => t.date === S.jDay); const St = stats(ts);
  ...
  <section class="grid2"><div class="panel">${!d ? '<p class="loading">Loading…</p>' : `
    ...
    <div class="panel"><h2>${fmtDate(S.jDay)} result</h2>
      <div class="kv"><span>Net P&L</span><b class="${cls(St.net)}">${money(St.net)}</b></div><div class="kv"><span>Closed trades</span><b>${St.n}</b></div>
  ...
  <section><h2>Trades that day</h2>${tradeTable(ts.slice().reverse(), false)}</section>`;
}
...
function vSettings() {
  ...
  const brokerHtml = b === undefined ? '<p class="loading">Loading…</p>' : b.connected ? `
    ...
    <p class="muted">Use read-only keys. Fills sync every weekday after the close, and stock charts load from Alpaca market data. Keys are encrypted and never shown again.</p>
  ...
  <section class="panel"><h2>Alpaca</h2><div id="broker">${brokerHtml}</div></section>`;
}
async function afterSettings() { if (S.broker !== undefined) return; try { S.broker = await api('/broker/alpaca'); } catch (e) { S.broker = { connected: false }; toast(e.message); } if (route() === 'settings') render(); }
...
document.addEventListener('click', e => {
  const el = e.target.closest('[data-range],[data-open],[data-tf],[data-tag],[data-emo],[data-wi],[data-bd],[data-ask],[data-score],#dr-close,#scrim,#rp-play,#save-plan,#rv-run,#j-save,#rep-run,#s-save,#al-save,#al-sync,#al-del,#signout');
  if (!el) return;
