...
{"headline":"one sentence on the week",
 ...
 "leaks":[{"label":"leak name","dollars":-123.45,"comment":"one sentence on why it happened or how to spot it"}],
 "rule":"one concrete, checkable rule for next week",
 "rule_reason":"one sentence linking the rule to the numbers",
 "last_rule_followed":"yes|no|unknown"}
Use at most 3 leaks, largest dollar loss first, only ones with negative dollars.
...
def run_for(sub):
    ...
    if not week:
        ...
        out = claude.json_call(os.environ.get("COACH_MODEL", "claude-sonnet-5"), SYSTEM, payload, 1200)
