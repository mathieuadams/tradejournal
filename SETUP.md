...
## Step 7. Connect Charles Schwab (automatic trade sync)

The app reads your executed trades from Schwab. The first sync covers the last year (or starts right after your last imported statement). After that it syncs every 15 minutes during market hours and once after the close.

1. **Create a Schwab developer account.**
   Go to https://developer.schwab.com and sign up. This is a separate login from your brokerage account.
2. **Create an app.**
   Go to **Dashboard → Apps → Create App** and fill in:
   - **API Product:** select **Accounts and Trading Production**
   - **App name:** anything, e.g. `tradejournal`
   - **Callback URL:** your app address followed by `/schwab`, e.g. `https://d2xuxuggx7zqe.cloudfront.net/schwab`. It must match exactly: https, no trailing slash. The deploy run summary prints the exact value.
3. **Wait until the app status is "Ready For Use".** Schwab reviews new apps, which can take a few days.
4. **Copy the key and secret.**
   Open the app and copy the **App Key** and the **Secret**.
5. **Add them to GitHub.**
   In the repo, go to **Settings → Secrets and variables → Actions → Secrets tab → New repository secret** and add:

| Name | Value |
|---|---|
| `SCHWAB_APP_KEY` | the App Key |
| `SCHWAB_APP_SECRET` | the Secret |

6. **Redeploy.**
   Go to **Actions → deploy → Run workflow**.
7. **Connect in the app.**
   1. Go to **Settings → Charles Schwab**.
   2. Set **Journal account name** to the same name you used for your statement imports (e.g. `Main`).
   3. Click **Connect Schwab**, log in to Schwab, and choose the accounts to share.
   4. You land back on Settings and the first sync starts.

> **Every 7 days** Schwab requires a new login. Settings shows the date. Click **Reconnect** before it to keep syncing without gaps. If it lapses, nothing is lost: the next sync after you reconnect catches up.

> A personal Schwab developer app is meant for your own accounts. Letting other people connect their Schwab accounts through your site needs Schwab's approval for a commercial app.
...
| Schwab status **login expired** | Click **Reconnect** in Settings → Charles Schwab. |
| Schwab page says the redirect URI is invalid | The callback URL in your Schwab app must exactly match the one in the deploy summary: https, ending in `/schwab`. |
